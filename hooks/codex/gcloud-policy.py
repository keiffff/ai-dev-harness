...
APPROVED_WRAPPER = os.environ.get("GCLOUD_READONLY_WRAPPER", os.path.expanduser("~/.local/bin/gcloud-readonly"))
RAW_COMMANDS = {"gcloud", "gsutil", "bq"}
...
def has_raw_fallback(command: str) -> bool:
    return bool(re.search(r"(^|[;&|]\s*|(?:^|\s)[A-Za-z_][A-Za-z0-9_]*=\S+\s+)(command\s+|\S*/)?(gcloud|gsutil|bq)(\s|$)", command))
...
def main() -> None:
    ...
    if is_raw_command(command):
        deny("Blocked raw Google Cloud CLI usage. Use " + APPROVED_WRAPPER + " for allowed read-only commands. For mutation or secret/token commands, present the command to the user instead of executing it.")
