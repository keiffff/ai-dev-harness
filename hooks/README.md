...
`japanese-output-review.py` は、日本語の文章を最終回答として送る直前に一度だけ止め、同じ Codex に推敲させます。読者、目的、媒体、情報の順序、意味、余計な補足、英語を直訳したような圧縮表現まで見直すための Stop hook です。
...
詳しい推敲基準は常時読む `AGENTS.md` と文章作成時の `codex-writing` に置きます。Stop hook の feedback は会話に表示されるため、推敲を求める一文だけを返します。
