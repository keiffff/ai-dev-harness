...
3. When all state is committed, create from the exact verified ref or commit. When post-creation transfer is required, create with the bootstrap prompt below and do not send the packet yet.
4. Resolve the destination checkout, transfer only manifested files without overwriting conflicts, and compare destination hashes with the manifest.
...
8. Verify creation, transfer, packet delivery, semantic scope preservation, and destination verification before reporting success. If the destination omits or reclassifies an unresolved workstream, correct and re-verify it. If any stage remains incomplete, report that precise stage instead of success.
...
If direct creation is unavailable or fails, do not claim success. Return the complete packet and, when useful, save a temporary Markdown backup outside the repository. Give one concise instruction for starting the destination manually.
