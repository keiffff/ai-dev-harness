...
| `codex-thread-handoff` | 長期化、compaction、工程境界、反復修正でfresh taskへの移行が有効なとき | 移行を一度だけ提案し、明示承認後は1回のtask作成でexact commitとcontinuation packetを渡し、60秒以内に完了または保留を確定する | ID未確定時にworktreeから推測したりtaskを再作成したりしない。source taskのarchiveは明示された設定・依頼がある場合のみ行う |
