...
- AI自身の不安、一般論としての安全性、丁寧さ、網羅性を根拠に、上限、閾値、timeout、retry回数、validation gate、approval、追加検証、確認工程、fallback、対象制限を新設しない。既存契約、観測された具体的な失敗、またはユーザーの明示要求に根拠がない制約は追加しない。
- 証拠が足りない場合は、裏付けられない主張や実装を狭めるか省く。AIの不確実性を、ユーザーやチームが従う新しい運用制約、将来作業、承認待ちへ変換しない。判断に不可欠な未確定事項だけをANDONとして確認する。
- reviewer、advisor、外部modelが提案した制約や検証項目は自動採用しない。現在の目的と契約に必要なものだけをCodex本体が採用し、それ以外は成果物へ残さない。
