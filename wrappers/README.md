...
- `bin/git-user-approved.example`: explicit-path add, explicitly requested branch creation/switch and single-upstream merge, no implicit push, no amend/commit-a fallback
...
- `bin/gemini-japanese-polish.example`: one stateless Gemini 3.8 Flash Japanese copy-editing pass with low thinking, structured output, no tools or retries, protected-span validation, token usage reporting, Keychain lookup, and a new-file-only output boundary
